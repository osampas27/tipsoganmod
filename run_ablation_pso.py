...
def train_eval(use_pso: bool):
    ...
    t.train_tipso(Xn, Xtr, ytr, Xv, yv, epochs=cfg.epochs_tipso, batch_size=cfg.batch_size, balance_strategy='class_weight', use_pso=use_pso)
    preds = t.dee.predict(Xte, verbose=0).argmax(axis=1)
    return compute_metrics(yte, preds)[0]
def main():
    os.makedirs('artifacts', exist_ok=True)
    rows = []
    for flag in [True, False]:
        m = train_eval(flag); m['use_pso'] = flag; rows.append(m)
    with open('artifacts/pso_vs_static.csv','w',newline='',encoding='utf-8') as f:
        cols=['use_pso','accuracy','precision','recall','f1','fp','fn','tp','tn']
        w=csv.DictWriter(f, fieldnames=cols); w.writeheader()
        for r in rows: w.writerow({k:r.get(k,'') for k in cols})
    print('Wrote artifacts/pso_vs_static.csv')
